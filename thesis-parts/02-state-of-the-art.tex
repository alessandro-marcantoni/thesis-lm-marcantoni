...
The premature development of these approaches, not completely adhering to the Web architectural style, and the lack of crucial initiatives such as the Web of Things~\cite{wot}, together with the shortage of real-world applications, have hindered their widespread acceptance.

\subsection{Hypermedia Multi-Agent Systems}
The missing gap between MAS and the Web is filled by the integration between the environment in MAS and the Web architecture~\cite{ciortea2019}.
Hypermedia MAS are systems composed of both people and autonomous agents situated in a shared \textit{hypermedia environment} that is distributed across the Web, that thus becomes an \textit{hypermedia application}.
According to this approach, all the entities, agents and artifacts, are represented as Web resources possibly interrelated by hyperlinks.

In contrast to typical endogenous environments, a hypermedia environment uses hypermedia to drive interaction in the MAS: agents navigate the environment at runtime to discover other entities in the MAS, as well as the means to interact with them, in a similar approach to the one used in the Web of Things Thing Description~\cite{wottd}.
This reduces coupling and enhances the scalability and evolvability of the systems.

% design principles for hypermedia MAS
